			...
			if(!plus.android.invoke(file,"isHidden")){
				...
				if(judgeIsFolder(filePath)){
					if(path.indexOf('MicroMsg')!=-1){
					   arr.push({'path':path+'/'+plus.android.invoke(file,"getName"),'fileName':plus.android.invoke(file,"getName"),'origin':'微信','isActive':false});						
					}else{
					   arr.push({'path':path+'/'+plus.android.invoke(file,"getName"),'fileName':plus.android.invoke(file,"getName"),'origin':'QQ','isActive':false});	
					}					
				}
			}						
		}
