<template>
	<view>
				...
				<view class="cognate-list" v-for="(listData,index) in listDatas" :key="index">
					...
					<view class="cognate-content">
						...
						<view class="work-cont">
							<view class="work-person">{{listData.origin}}</view>
						</view>
					</view>
				</view>
			</view>
			<view v-show="current === 1">
				<view class="cognate-list" v-for="(listDatasWx,index) in listDatasWxs" :key="index">
					<view class="cognate-radio"><radio value="cb" :checked="listDatasWx.isActive"   @click.prevent="getChange(listDatasWx.path)"/></view>
					<view class="cognate-content">
						<view class="work-name" :class="{activeColor:listDatasWx.isActive}">{{listDatasWx.fileName}}</view>
						<view class="work-cont">
							<view class="work-person">{{listDatasWx.origin}}</view>
						</view>
					</view>
				</view>
			</view>
			<view v-show="current === 2">
				<view class="cognate-list" v-for="(listDatasQq,index) in listDatasQqs" :key="index">
					<view class="cognate-radio"><radio value="cb" :checked="listDatasQq.isActive"   @click.prevent="getChange(listDatasQq.path)"/></view>
					<view class="cognate-content">
						<view class="work-name" :class="{activeColor:listDatasQq.isActive}">{{listDatasQq.fileName}}</view>
						<view class="work-cont">
							<view class="work-person">{{listDatasQq.origin}}</view>
						</view>
					</view>
				</view>
			</view>			
		...
		<!-- 无数据页面 -->
		<view class="no_data"  v-if="isData"><image class="no_img" src="@/static/nodata.png" mode=""></image></view>			
		<view class="uniBtn">
			<button class="btn" type="default" v-if="false">返回</button>
			<button class="btn btn-submit" :class="{disbled:disbled}"  @click="savePath">选定保存</button>
		</view>				
	</view>
</template> 
<script>
	import uniSegmentedControl from '@/components/uni-segmented-control/uni-segmented-control.vue';
	var fileFun = require('@/common/getDriFile.js');
	...
	export default {
		...
		onLoad(opt){
            ...
            setTimeout(()=>{
				...
				this.listDatasWx = fileFun.readSonFilenum(this.getSDCardPath+'/tencent/MicroMsg/Download/');   //ss微信下载 
		...
		methods: {
			...
			savePath(){
				...
				this.$eventHub.$emit('_singlefile',arr);
				uni.navigateBack()
			}
		},
		onBackPress(){
